...
describe("production registration-request migration runner and CLI", () => {
  ...
  it("applies the current plan through 0025, re-applies unchanged, and creates zero rows", async () => {
  ...
  it("uses the production CLI for apply and unchanged reapply", async () => {
    ...
    expect(first.stdout).toContain("Migrations complete: 25 applied, 0 unchanged.");
    expect(first.stdout).toContain("applied 0013_create_price_table_items.sql");
    expect(second.stdout).toContain("Migrations complete: 0 applied, 25 unchanged.");
