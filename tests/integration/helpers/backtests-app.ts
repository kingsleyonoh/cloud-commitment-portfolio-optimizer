...
export interface BacktestsHarness {
...
export async function createBacktestsHarness(prefix: string): Promise<BacktestsHarness> {
  ...
  const tenantA = await insertTenant(pool, "Backtest tenant A");
  const tenantB = await insertTenant(pool, "Backtest tenant B");
  ...
  const app = buildApp({
    ...
    backtests: {
      ...
      service: createBacktestsService(createBacktestsRepository(pool), objectStore, {
        maxMonths: 12,
...
export async function closeBacktestsHarness(harness?: BacktestsHarness): Promise<void> {
  if (!harness) return;
...
export function backtestsAuthorization(
  harness: BacktestsHarness,
