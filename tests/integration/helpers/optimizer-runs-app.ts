...
export interface OptimizerRunsHarness {
...
export async function createOptimizerRunsHarness(prefix: string): Promise<OptimizerRunsHarness> {
  ...
  const tenantA = await insertTenant(pool, "Optimizer run tenant A");
  const tenantB = await insertTenant(pool, "Optimizer run tenant B");
...
export async function closeOptimizerRunsHarness(harness?: OptimizerRunsHarness): Promise<void> {
  if (!harness) return;
...
export function optimizerRunsAuthorization(
  harness: OptimizerRunsHarness,
