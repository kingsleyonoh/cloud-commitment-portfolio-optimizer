...
export interface OptimizerPoliciesHarness {
...
export async function createOptimizerPoliciesHarness(
  prefix: string,
): Promise<OptimizerPoliciesHarness> {
  ...
  const tenantA = await insertTenant(pool, "Optimizer policy tenant A");
  const tenantB = await insertTenant(pool, "Optimizer policy tenant B");
  ...
  const app = buildApp({
    ...
    optimizerPolicies: {
      ...
      service: createOptimizerPoliciesService(createOptimizerPoliciesRepository(pool)),
    },
  });
  return {
    database,
...
export async function closeOptimizerPoliciesHarness(
  harness?: OptimizerPoliciesHarness,
): Promise<void> {
  if (!harness) return;
...
export function optimizerPoliciesAuthorization(
  harness: OptimizerPoliciesHarness,
