...
import type { PriceTableCursorBoundary } from "./price-tables-types.js";

const CURSOR_PATTERN = /^[A-Za-z0-9_-]{38,512}$/u;

export function encodePriceTableCursor(row: PriceTableCursorBoundary): string {
  return Buffer.from(JSON.stringify({ created_at: row.createdAt, id: row.id }), "utf8").toString(
    "base64url",
  );
}

export function decodePriceTableCursor(value: string): PriceTableCursorBoundary {
