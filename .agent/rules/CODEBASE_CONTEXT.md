...
| `apps/api/` | Fastify API, auth middleware, REST routes | Current for health, registration, auth/session, tenant profile, users, API-key metadata/rotation, cloud-account management, imports, price tables, forecast model/run queueing, and optimizer policy management |
| `apps/web/` | HTMX server-rendered UI templates/routes | Planned — Phase 1 |
| `apps/worker/` | Queue consumers invoking forecast/optimizer/replay workers | Current for one forecast processor pass behind queue readiness; optimizer/replay remain planned |
...
| API routes | `apps/api/routes/` | Current for foundation/auth/tenant/user/API-key/cloud-account/import/price-table/forecast/optimizer-policy routes |
| HTMX views | `apps/web/views/` | Planned — Phase 1 |
| Workers | `apps/worker/` | Current for forecast processor startup pass; optimizer/replay remain planned |
...
| Optimizer | `core/optimizer/` | Current for PRD §5.5 economic formula CLI and golden fixtures; optimizer routes/workers remain planned |
