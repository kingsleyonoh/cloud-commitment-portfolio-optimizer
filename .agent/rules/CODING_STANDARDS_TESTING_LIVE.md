# Cloud Commitment Portfolio Optimizer — Coding Standards: Live & Integration Testing
...
- Your database (local PostgreSQL 16, local Redis 7, local DuckDB temp storage, local filesystem object storage) — validates schema, column names, constraints, query behavior
...
> This section applies to Fastify API, HTMX server-rendered UI, workers, and CLI/binary boundaries. For HTMX/server-rendered UI, verify interactions through Fastify route tests and Playwright E2E rather than React component tests.
